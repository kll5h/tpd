$(document).ready(function() {
    var cardCreator = new $.fn.dataTable.Editor({
        ...
        i18n: {
            create: {
        	title: '<i class="fa fa-credit-card"></i><span>Create new card</span>'
            }
        },
        fields: [{
    ...
    var upholdCardsTable = $('#upholdCardsTable').DataTable({
        ...
        },
        {
            ...
            searchable: false,
            orderable: false,
            defaultContent: '<button class="btn btn-link btn-xs create-transaction-btn"><i class="fa fa-exchange"></i><span>Send or Transfer</span></button>'
        }],
        columns: [{
            "data": "id"
        },
    ...
    var transactionCreator = new $.fn.dataTable.Editor({
        ...
        fields: [{
            ...
            type: "radio",
            options: [
                { label: "My cards", value: "my cards", checked: true },
                { label: "My contacts", value: "my contacts", checked: false },
    ...
    $('#upholdCardsTable').on('click', 'button.create-transaction-btn', function (e) {
        ...
        transactionCreator.create('<span><img src="/images/uphold/currencies/' + card.currency + '.png" height="6%" width="6%" /> ' + card.label + '</span>', {
        	...
        	this.submit(function (json) {
        	    ...
        	    var that = this;
                    $.post("/uphold/commitTransaction", {id: transaction.id, cardId: card.id, message: transaction.message}).done(function(json) {
                	var table = $('#upholdTransactionsTable').DataTable();
                	if (json.successful) {
                	    table.row.add(json.data).draw();
                	    that.message('<div class="alert alert-success">Transaction has been created successfully!</div>');
                    	    setTimeout( function () {
                                that.close();
                            }, 1000);
                	} else {
                	    that.message('<div class="alert alert-danger">Transaction has been created failure!</div>');
