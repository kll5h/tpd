...
/**
 * Simple client that connects to the given host, opens a channel, and pays one
 * cent.
 */
public class Client {
    ...
    public void run(final String host) throws Exception {
        ...
        appKit = new WalletAppKit(params, new File("."), "payment_channel_example_server") {
            ...
            protected List<WalletExtension> provideWalletExtensions() {
                ...
                // after a restart.
                // We should not send a PeerGroup in the
                // StoredPaymentChannelClientStates constructor
        ...
        // Create the object which manages the payment channels protocol, client
        // side. Tell it where the server to
        // connect to is, along with some reasonable network timeouts, the
        // wallet and our temporary key. We also have
        // to pick an amount of value to lock up for the duration of the
        // channel.
        //
        // Note that this may or may not actually construct a new channel. If an
        // existing unclosed channel is found in
        ...
        // Do this twice as each one sends 1/10th of a bitcent 5 times, so to
        // send a bitcent, we do it twice. This
        // demonstrates resuming a channel that wasn't closed yet. It should
        // close automatically once we run out
        // of money on the channel.
        log.info("Round one ...");
        openAndSend(timeoutSecs, server, channelID, 5);
        log.info("Round two ...");
        log.info(appKit.wallet().toString());
        openAndSend(timeoutSecs, server, channelID, 10); // 4 times because the
                                                        // opening of the
                                                        // channel made a
                                                        // payment.
        log.info("Stopping ...");
    ...
    private void openAndSend(int timeoutSecs, InetSocketAddress server, String channelID, final int times) throws IOException, ValueOutOfRangeException,
            InterruptedException {
        PaymentChannelClientConnection client = new PaymentChannelClientConnection(server, timeoutSecs, appKit.wallet(), myKey, channelSize, channelID);
        // Opening the channel requires talking to the server, so it's
        // asynchronous.
        ...
        Futures.addCallback(client.getChannelOpenFuture(), new FutureCallback<PaymentChannelClientConnection>() {
            ...
            public void onSuccess(PaymentChannelClientConnection client) {
                // By the time we get here, if the channel is new then we
                // already made a micropayment! The reason is,
                // we are not allowed to have payment channels that pay nothing
                // at all.
                log.info("Success! Trying to make {} micropayments. Already paid {} satoshis on this channel", times, client.state().getValueSpent());
                final Coin MICROPAYMENT_SIZE = CENT.divide(10);
                for (int i = 0; i < times; i++) {
                    try {
                        // Wait because the act of making a micropayment is
                        // async, and we're not allowed to overlap.
                        // This callback is running on the user thread (see the
                        // last lines in openAndSend) so it's safe
                        // for us to block here: if we didn't select the right
                        // thread, we'd end up blocking the payment
                ...
                if (client.state().getValueRefunded().compareTo(MICROPAYMENT_SIZE) < 0) {
                    // Now tell the server we're done so they should broadcast
                    // the final transaction and refund us what's
                    // left. If we never do this then eventually the server will
                    // time out and do it anyway and if the
                    // server goes away for longer, then eventually WE will time
                    // out and the refund tx will get broadcast
                    // by ourselves.
                	System.out.println("Settling channel for good");
                    log.info("Settling channel for good");
                    client.settle();
                } else {
                    // Just unplug from the server but leave the channel open so
                    // it can resume later.
    ...
    private void waitForSufficientBalance(Coin amount) {
        ...
        if (!balanceFuture.isDone()) {
            System.out.println("Please send " + amountPlusFee.toFriendlyString() + " to " + myKey.toAddress(params));
